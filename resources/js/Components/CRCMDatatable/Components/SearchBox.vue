<template>
    <div class="flex flex-col gap-0.5">
        ...
        <div class="flex flex-row items-center justify-between gap-0.5 bg-white rounded focus-within:ring-1 border-gray-200 border">
            <input autocomplete="off" class="border-0 py-1.5 w-full rounded-l active:ring-0 focus:ring-0" type="text" v-model="search" id="dtSearch" @keyup="typing($event.target.value)" placeholder="Find it here" @keyup.capture.delete="!search.length?searchBy():null" @keyup.capture.enter="searchBy" />
