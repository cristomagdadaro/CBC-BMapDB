...
<template>
    <div v-if="!sublinks" class="flex items-center hover:bg-cbc-dark-green duration-400 ease-in-out">
    ...
    <div v-else class="flex items-center hover:bg-cbc-yellow-green duration-400 ease-in-out" @mouseleave="closeDropdown()">
        <div>
            <div @click="toggleDropdown()" class="flex items-center px-3 py-1 text-white whitespace-nowrap sm:text-sm text-xs">
                <slot name="trigger" />
            </div>
            ...
            <transition
                ...
                <div class="absolute flex flex-col bg-gray-500 z-50 mt-2 rounded-md shadow-md p-2" v-show="showDropdown">
                    <slot name="content" />
                </div>
            </transition>
        </div>
